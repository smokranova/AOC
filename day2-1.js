...
let input = fileHandler.getInputArr('input2.txt')
//2286 for power
...
let processGame = function(game, sum){
    ...
    let x = [0,0,0]
    for(cubeSet of cubeSets){
        x = processColors(cubeSet.split(', '), x)
    }
    let power = getPower(x)
    console.log(power)
    return power
}
